...
from Lunodin.status import status

API_REQUIRED_PARAMS = ["number_of_cycles", "phone_code", "phone"]

os.chdir(os.path.join(pkg_resources.get_distribution("Lunodin").location, "Lunodin"))
...
def retrieve_installed_version():
    package_info = subprocess.run(
        [sys.executable, "-m", "pip", "show", "Lunodin"], stdout=subprocess.PIPE
    )
...
def update(force: bool = False):
    if force:
        subprocess.run([sys.executable, "-m", "pip", "install", "Lunodin", "--force-reinstall"],)
